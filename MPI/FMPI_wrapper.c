...
/*将c中的request转换为fortran中的request,对LA_MPI进行特殊处理*/
#ifdef LA_MPI
MPI_Fint MPI_Request_c2f_ (MPI_Request c_request)
{
	MPI_Fint f_request;
	f_request = MPI_Request_c2f (c_request);
	/*LA_MPI 不能正确的翻译MPI_REQUEST_NULL,所以我们要先检查是否是LA_MPI，进行相应的处理*/
	if  (c_request  =  =  MPI_REQUEST_NULL)
		f_request  =  -1;
	return f_request;
}
#else
#define MPI_Request_c2f_ MPI_Request_c2f
#endif

/*
	相当于每一个进程都执行了一次 MPI_Gather 操作,即每一个进程都从其它的进程收集消息
*/
...
/*
	相当于每一个进程都执行了一次 MPI_Gatherv 操作,即每一个进程都从其它的进程收集消息到指定
	的位置,接收数据存放的位置用一个数组来指定。
*/
...
void pi_comm_dup (comm, comm_out, ierr)
MPI_Fint *comm;
MPI_Fint * comm_out;
MPI_Fint *ierr;
{
...
{
	MPI_Group local_group;
	*ierr = MPI_Group_difference (MPI_Group_f2c (*group1), MPI_Group_f2c (*group2), &local_group);
...
MPI_Fint *ierr;
{
	...
	*request = MPI_Request_c2f_ (local_request);
}
...
{
	int i;
	MPI_Request *local_requests;
	if (*count > 0)
	{
		/*申请request数组*/
		local_requests = alloc_requests (*count);
		/*将请求数组中的每一个请求由Fortran类型请求转换为C类型请求，并暂时保存在本地数组中*/
		for (i = 0; i < *count; i++)
		{
			local_requests[i] = MPI_Request_f2c (array_of_requests[i]);
		}
	}
	/*调用C中的对应函数完成相应功能，并将返回值保存在ierr中*/
	*ierr = MPI_Startall (*count, local_requests);

	/*将请求数组中的每一个请求由C类型请求转换为Fortran类型请求*/
	for (i = 0; i < *count; i++)
	{
		array_of_requests[i] = MPI_Request_c2f_ (local_requests[i]);
	}
}
...
MPI_Fint *ierr;
{
	...
	*request = MPI_Request_c2f_ (local_request);						/*调用C中的对应函数完成相应功能，并将返回值保存在ierr中*/
	MPI_Status_c2f (&local_status, status);								/*将C类型request转换为Fortran类型request*/
}
...
MPI_Fint *ierr;
{
	int i;
	MPI_Request *local_requests;
	MPI_Status *local_statuses;

	if (*count > 0)
	{
		local_requests = alloc_requests (*count);							/*申请request数组*/
		local_statuses = alloc_statuses (*count);							/*申请status数组*/
	...
	/*调用C中的对应函数完成相应功能，并将返回值保存在ierr中。如果所有请求都已完成，则flag值为TRUE*/
	*ierr = MPI_Testall (*count, local_requests, flag, local_statuses);
	...
	{	
		...
		for (i = 0; i < *count; i++)
		{
			MPI_Status_c2f ( &(local_statuses[i]), &(array_of_statuses[i]));
		}
	}
...
MPI_Fint *ierr;
{
	...
	*ierr = MPI_Testany (*count, local_requests, index, flag, &local_status);
	if (*ierr == MPI_SUCCESS)
	{
		if (*flag && *index >= 0)
		{
			array_of_requests[*index] = MPI_Request_c2f_ (local_requests[*index]);
			(*index)++; 
		}
		MPI_Status_c2f (&local_status, status);
	}
...
MPI_Fint *ierr;
{
	...
	if (*incount > 0)
	{
		local_requests = alloc_requests (*incount);
		local_statuses = alloc_statuses (*incount);
		for (i = 0; i < *incount; i++)
		{
			local_requests[i] = MPI_Request_f2c (array_of_requests[i]);
		}
	...
	*ierr = MPI_Testsome (*incount, local_requests, outcount, array_of_indices, local_statuses);
	if (*ierr == MPI_SUCCESS)
	{
		for (i = 0; i < *incount; i++)
		{
			if (i < *outcount)
			{
				array_of_requests[array_of_indices[i]] = MPI_Request_c2f_ (local_requests[array_of_indices[i]]);
			}
			else 
			{
				found = 0;
...
MPI_Fint *ierr;
{
	...
	*ierr = MPI_Type_contiguous (*count, MPI_Type_f2c (*old_type), &local_new_type);
...
MPI_Fint *ierr;
{
	...
	*ierr = MPI_Type_lb (MPI_Type_f2c (*datatype), &c_displacement);		/*调用C中的对应函数完成相应功能，并将返回值保存在ierr中*/
	*displacement = c_displacement;											/*将 C 数据类型句柄转换成 Fortran 数据类型句柄*/
}
...
MPI_Fint *ierr;
{
	MPI_Aint *c_indices, *local_types;
	int i;
	MPI_Datatype local_new_type; 
	...
	for (i = 0; i < *count; i++)
	{
		local_types[i] = MPI_Type_f2c(old_types[i]);
	}
	c_indices =  (MPI_Aint *) malloc  (*count * sizeof (MPI_Aint));
	for (i = 0; i < *count; i++)
	{
...
MPI_Fint *ierr;
{
	...
	{
		local_requests = alloc_requests (*count);
		local_statuses = alloc_statuses (*count);
