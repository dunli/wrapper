...
void create_database_procedure (MYSQL* conn, char * tablename)
{
	...
	/*建立insertdata存储过程*/
	snprintf_d (sql, 1024, "CREATE PROCEDURE insertdata (mpi_rank int,pid int, hostname text, omp_rank int, tid int, eid int,name text,type int,mpi_comm text,time bigint(20),finish int)\nBEGIN\n"
			"insert into `%s_trace` (mpi_rank, pid, hostname, omp_rank, tid, eid, name, type, comm, time, finish) values (mpi_rank, pid, hostname, omp_rank, tid, eid, name, type, mpi_comm, time, finish);\n"
			"END;"
			,tablename);
	...
	snprintf_d (sql, 1024, "CREATE PROCEDURE insert_comm_data (mpi_rank int,pid int, hostname text, omp_rank int, tid int, eid int,name text, tag int ,src_rank int,dst_rank int,send_size int,send_type int,recv_size int,recv_type int,time bigint(20),finish int)\nBEGIN\n"
			"insert into `%s_comm` (mpi_rank, pid, hostname, omp_rank, tid, eid, name, tag, src_rank, dst_rank, send_size, send_type, recv_size, recv_type, time, finish) values (mpi_rank, pid, hostname, omp_rank, tid, eid, name, tag, src_rank, dst_rank, send_size, send_type, recv_size, recv_type, time, finish);\n"
			"END;"
			,tablename);
...
void create_tables (MYSQL* conn, char * tablename, char * exp_type, char * owner)
{
	...
	/*建ompi_trace表*/
	snprintf_d(sql, 1024, sql_ompi_trace, tablename);
	if (mysql_query (conn, sql))
	{
		printf_d ("[MRNFE] %s\n", mysql_error (conn));
		exit (-1);
	...
	/*建ompi_comm_trace表*/
	snprintf_d(sql, 1024, sql_ompi_comm_trace, tablename);
	if (mysql_query (conn, sql))
	{
		printf_d ("[MRNFE] %s\n", mysql_error (conn));
		exit (-1);
...
void data_recving (MYSQL* conn, char * tablename, int nr_be)
{
	...
	int tag;
	PacketPtr pack;
	int rank, eid, data, finish, nr_record, mpi_rank, omp_rank, tid, type, src_rank, dst_rank, sendsize, sendtype, recvsize, recvtype;
	...
	while (nr_exited != nr_be)
	{
		...
		switch (tag)
		{
			case PROT_DATA:
				...
				snprintf_d(sql, 2048, "CALL insertdata ( -1,%d, \"%s\" ,-1, %d, %d, \"%s\", 7,\"FFFFFFFF\", '%llu', %d );"
						, pid, hostname,tid, eid, ename, time, finish);
		...
		//		printf_d("[DEBUG] events: %s\n", ename);
				...
				break;
			case PROT_OMPIDATA:											/*MPI wrapper treace 数据*/
				pack->unpack("%d %ud %s %d %d %d %s %d %d %d %d %d %d %d %d %d %uld %d",
						&mpi_rank,
						&pid,
						&hostname,
						&omp_rank,
						...
						&time,
						&finish);
				snprintf_d (sql, 2048,"CALL insertdata ( %d, %u, \"%s\", %d, %d, %d, \"%s\", %d, \"%x\", %llu, %d);",
						mpi_rank,
						pid,
						hostname,
						omp_rank,
				...
				if (type != 0)
				{
					snprintf_d (sql, 2048,"CALL insert_comm_data ( %d, %u, \"%s\", %d, %d, %d, \"%s\", %d, %d, %d, %d, %d, %d, %d, %llu, %d);",
							mpi_rank,
							pid,
							hostname,
							omp_rank,
